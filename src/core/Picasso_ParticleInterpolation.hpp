                   ...
                   SplineMembers... )
{
    ...
    typename PositionVector::value_type x[3] =
        { position(0), position(1), position(2) };
    Cajita::SplineData<
        typename PositionVector::value_type,
        Order::value,
        typename Location::entity_type,
        Cajita::SplineDataMemberTypes<
            typename SplineMembers::spline_data_member...>> sd;
...
template<class ViewType,
         ...
         typename std::enable_if_t<
             !LinearAlgebra::is_vector<Scalar>::value,int> = 0>
KOKKOS_INLINE_FUNCTION
void value( const SplineDataType& sd,
            const ViewType& view,
            Scalar& result )
{
...
template<class ViewType,
         ...
         typename std::enable_if_t<
             LinearAlgebra::is_vector<ResultVector>::value,int> = 0>
KOKKOS_INLINE_FUNCTION
void gradient( const SplineDataType& sd,
               const ViewType& view,
               ResultVector& result )
{
    // FIXME - replace this with eval_type once we update Cajita to use
...
KOKKOS_INLINE_FUNCTION
void divergence( const SplineDataType& sd,
                 const ViewType& view,
                 Scalar& result )
{
            ...
            const ScatterViewType& view )
{
    // FIXME - replace this with eval_type once we update Cajita to use
    // operator() of the input point data for interpolation.
    typename ValueVector::value_type v[3] = { value(0), value(1), value(2) };
