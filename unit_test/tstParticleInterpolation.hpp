...
struct ScalarValueP2G
{
                     ...
                     ParticleViewType& particle ) const
    {
        ...
        auto node_scalar = scatter_deps.get(FieldLocation::Node(),NodeScalar());

        // Get particle data.
        auto particle_scalar = get( particle, ParticleScalar() );

        // Node Interpolant
        auto spline = createSpline( FieldLocation::Node(),
...
struct VectorValueP2G
{
    ...
    {
        // Get output dependencies.
        auto node_vector = scatter_deps.get(FieldLocation::Node(),NodeVector());
...
struct ScalarValueG2P
{
    ...
    void operator()( const LocalMeshType& local_mesh,
                     ...
                     const ScatterDependencies&,
                     const LocalDependencies&,
                     ParticleViewType& particle ) const
    {
        // Get input dependencies.
        auto node_scalar = gather_deps.get(FieldLocation::Node(),NodeScalar());
...
struct VectorValueG2P
{
    ...
    {
        // Get input dependencies.
        auto node_vector = gather_deps.get(FieldLocation::Node(),NodeVector());
...
void interpolationTest()
{
    ...
    int ppc = 1;
    initializeParticles(
        InitUniform(), TEST_EXECSPACE(), ppc, particle_init_func, particles );
    ...
    // Make mirror
    auto particles_host = Cabana::create_mirror_view(
    ...
    auto scalar_n_host = Kokkos::create_mirror_view(
        Kokkos::HostSpace(), scalar_n );
    auto vector_n_host = Kokkos::create_mirror_view(
        Kokkos::HostSpace(), vector_n );
    ...
    p2gsv_op->apply( FieldLocation::Particle(),
                     TEST_EXECSPACE(),
                     *fm,
                     particles,
                     ScalarValueP2G() );
    ...
    p2gvv_op->apply( FieldLocation::Particle(),
                     TEST_EXECSPACE(),
                     *fm,
                     particles,
                     VectorValueP2G() );
    ...
    Kokkos::deep_copy( vector_n, 0.0 );
    p2gsg_op->apply( FieldLocation::Particle(),
                     TEST_EXECSPACE(),
                     *fm,
                     particles,
                     ScalarGradientP2G() );
    ...
    for ( int i = node_space.min( Dim::I ); i < node_space.max( Dim::I ); ++i )
        for ( int j = node_space.min( Dim::J ); j < node_space.max( Dim::J );
            ...
            for ( int k = node_space.min( Dim::K );
                  k < node_space.max( Dim::K ); ++k )
                for ( int d = 0; d < 3; ++d )
                    EXPECT_FLOAT_EQ( vector_n_host( i, j, k, d ) + 1.0,
                                     1.0 );

    // Interpolate a vector point divergence value to the grid.
    Kokkos::deep_copy( scalar_n, 0.0 );
    p2gvd_op->apply( FieldLocation::Particle(),
                     TEST_EXECSPACE(),
                     *fm,
                     particles,
                     VectorDivergenceP2G() );
    ...
    Kokkos::deep_copy( vector_n, 0.0 );
    p2gtd_op->apply( FieldLocation::Particle(),
                     TEST_EXECSPACE(),
                     *fm,
                     particles,
                     TensorDivergenceP2G() );
    ...
    g2psv_op->apply( FieldLocation::Particle(),
                     TEST_EXECSPACE(),
                     *fm,
                     particles,
                     ScalarValueG2P() );
    ...
    g2pvv_op->apply( FieldLocation::Particle(),
                     TEST_EXECSPACE(),
                     *fm,
                     particles,
                     VectorValueG2P() );
    ...
    Cabana::deep_copy( vector_p, 0.0 );
    g2psg_op->apply( FieldLocation::Particle(),
                     TEST_EXECSPACE(),
                     *fm,
                     particles,
                     ScalarGradientG2P() );
    ...
    g2pvg_op->apply( FieldLocation::Particle(),
                     TEST_EXECSPACE(),
                     *fm,
                     particles,
                     VectorGradientG2P() );
    ...
    Cabana::deep_copy( scalar_p, 0.0 );
    g2pvd_op->apply( FieldLocation::Particle(),
                     TEST_EXECSPACE(),
                     *fm,
                     particles,
                     VectorDivergenceG2P() );
...
// RUN TESTS
//---------------------------------------------------------------------------//
TEST( TEST_CATEGORY, interpolation_test )
{
    interpolationTest();
}
