...
void particleTest()
{
    ...
    for ( std::size_t p = 0; p < num_p; ++p )
    {
        ...
        for ( int d = 0; d < 3; ++d )
            EXPECT_DOUBLE_EQ(
                ParticleAccess::get(particle,Field::LogicalPosition(),d), 1.23 );
        ...
        for ( int i = 0; i < 3; ++i )
            for ( int j = 0; j < 3; ++j )
                EXPECT_DOUBLE_EQ(
                    ParticleAccess::get(particle,Field::DeformationGradient(),i,j),
                    -1.2 );
    }
    ...
    for ( std::size_t p = 0; p < num_p; ++p )
    {
        ...
        for ( int d = 0; d < 3; ++d )
            EXPECT_DOUBLE_EQ(
                ParticleAccess::get(particle,Field::LogicalPosition(),d), 2.23 );
