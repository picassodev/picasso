...
void checkGridVelocity( std::integral_constant<int,1>,
                            ...
                            const int array_dim )
{
    ...
    EXPECT_NEAR( pb_host(array_dim,0), mathematica_b[array_dim][0], near_eps );
    EXPECT_NEAR( pb_host(array_dim,1), mathematica_b[array_dim][1], near_eps );
    EXPECT_NEAR( pb_host(array_dim,2), mathematica_b[array_dim][2], near_eps );
}

//---------------------------------------------------------------------------//
// Check the grid momentum. Computed in mathematica.
template<class GridMomentum>
void checkGridMomentum( std::integral_constant<int,1>,
                        const int cx, const int cy, const int cz,
...
void checkGridMass( std::integral_constant<int,1>,
                    const int cx, const int cy, const int cz,
                    const GridMass& gm_host, const double near_eps )
{
    double mathematica[8] =
        {
            0.019390335999999897,
            0.009124863999999963,
            0.015235263999999998,
            0.0071695360000000085,
            0.031636863999999966,
            0.014887936000000006,
            0.024857536000000104,
            0.011697664000000064
        };
    ...
    for ( int i = cx; i < cx+2; ++i )
        for ( int j = cy; j < cy+2; ++j )
            for ( int k = cz; k < cz+2; ++k, ++n )
                EXPECT_NEAR( gm_host(i,j,k,0),
                             mathematica[n], near_eps );
}
...
// Check the grid velocity. Computed in mathematica.
template<class GridVelocity>
void checkGridVelocity( std::integral_constant<int,2>,
                        const int cx, const int cy, const int cz,
                        const GridVelocity& gv_host,
                        const double near_eps,
                        const int test_dim, const int array_dim )
...
// Check the grid momentum. Computed in mathematica.
template<class GridMomentum>
void checkGridMomentum( std::integral_constant<int,2>,
                        const int cx, const int cy, const int cz,
                        const GridMomentum& gv_host, const double near_eps,
                        const int test_dim, const int array_dim )
{
...
// Check the grid mass. Computed in mathematica.
template<class GridMass>
void checkGridMass( std::integral_constant<int,2>,
                    const int cx, const int cy, const int cz,
                    const GridMass& gm_host, const double near_eps )
{
    double mathematica[27] =
        {
            0.0003507323975679963,
            0.0010886054000639896,
            0.00007767084236799938,
            0.0016695577904639904,
            0.005181983868671976,
            0.0003697290608639989,
            0.00021652357196799997,
            0.0006720472112640006,
            0.000047949856768000124,
            0.0035733899120639796,
            0.011091109865471948,
            0.0007913389424639975,
            0.017010065244671985,
            0.05279594645145601,
            0.003766934863872007,
            0.0022060213232640105,
            0.00684706272307204,
            0.0004885306736640036,
            0.000933667130367998,
            0.002897921854463997,
            0.0002067636551680001,
            0.004444446084864011,
            0.01379469943987205,
            0.0009842371952640053,
            0.0005763965447680048,
            0.0017890231856640168,
            0.0001276449095680014
        };
...
// Check the grid velocity. Computed in mathematica.
template<class GridVelocity>
void checkGridVelocity( std::integral_constant<int,3>,
                        const int cx, const int cy, const int cz,
                        const GridVelocity& gv_host,
                        const double near_eps,
                        const int test_dim, const int array_dim )
...
// Check the grid momentum. Computed in mathematica.
template<class GridMomentum>
void checkGridMomentum( std::integral_constant<int,3>,
                        const int cx, const int cy, const int cz,
                        const GridMomentum& gv_host, const double near_eps,
                        const int test_dim, const int array_dim )
{
    double mathematica[64][3] =
        {
            {-0.0018334085691580541, -0.0018242775143538393, -0.0018113871537709235},
            ...
            {-0.00018999100203442195, -0.00018903942457467065, -0.0001876960774103205},
            ...
            {-0.003096655598189641, -0.0030823721831455078, -0.0030621876705793404},
            {-0.0009981801369304203, -0.0009937741692505194, -0.0009875447214025821},
            ...
            {-0.00010350242539818213, -0.00010304326491535914, -0.0001023940735255477},
            ...
            {-0.0008964101112975921, -0.0008920395576665961, -0.0008859674147730544},
            ...
            {-0.000484089505335131, -0.00048197999974876414, -0.0004790451252094456}
        };
...
// Check the grid mass. Computed in mathematica.
template<class GridMass>
void checkGridMass( std::integral_constant<int,3>,
                    const int cx, const int cy, const int cz,
                    const GridMass& gm_host, const double near_eps )
{
    double mathematica[64] =
        {
            1.8797178095437055e-6,
            0.000020827258025711575,
            0.000012965959057119188,
            1.9589161784782678e-7,
            0.00003311632582608277,
            0.0003669286205309665,
            0.0002284305243069647,
            3.4511619830967442e-6,
            0.00002831359202043021,
            0.00031371436906658576,
            0.0001953021208997269,
            2.9506531883696727e-6,
            9.11772742165714e-7,
            0.000010102434559847897,
            6.289246175301007e-6,
            9.501885690796734e-8,
            0.00008250944453604763,
            0.0009142039736951518,
            0.0005691354703606992,
            8.598582455211928e-6,
            0.0014536275791567563,
            0.01610618174210809,
            0.010026864447393296,
            0.00015148734388932547,
            0.001242813542843995,
            0.013770363936139954,
            0.008572706727750885,
            0.00012951771502872435,
            0.00004002189164631248,
            0.0004434422335963082,
            0.0002760638888666562,
            4.170813865848138e-6,
            0.00011298504437458089,
            0.001251873371783976,
            0.0007793507365784994,
            0.000011774545638059265,
            0.0019905379009476495,
            0.022055143736207216,
            0.013730376333241364,
            0.00020744054656730962,
            0.0017018578185459842,
            0.018856570773533286,
            0.011739112479687522,
            0.0001773562391808549,
            0.00005480433457090743,
            0.0006072315808471222,
            0.0003780305504322166,
            5.711341196886726e-6,
            8.16426203005055e-6,
            0.00009045995682227375,
            0.000056315627098795606,
            8.508247831031677e-7,
            0.0001438356119966227,
            0.001593697409861213,
            0.0009921524638619055,
            0.000014989585455377672,
            0.00012297568448471193,
            0.0013625695828637832,
            0.0008482643947694685,
            0.000012815703329161863,
            3.960143134838734e-6,
            0.00004387835369185446,
            0.000027316362852951293,
            4.126996305795691e-7
        };
...
void collocatedTest()
{
    ...
    checkGridVelocity( std::integral_constant<int,Order>(),
                       cx, cy, cz, gv_host, near_eps, 0, 0 );
    checkGridVelocity( std::integral_constant<int,Order>(),
                       cx, cy, cz, gv_host, near_eps, 1, 1 );
    checkGridVelocity( std::integral_constant<int,Order>(),
                       cx, cy, cz, gv_host, near_eps, 2, 2 );

    // Do G2P.
    auto gv_wrapper = createViewWrapper(
        FieldLayout<Location,Foo>(), gv_view );
    Kokkos::parallel_for(
        "g2p",
    ...
    auto pu_host = Kokkos::create_mirror_view_and_copy(
        Kokkos::HostSpace(), pu );
    auto pb_host = Kokkos::create_mirror_view_and_copy(
        Kokkos::HostSpace(), pb );
    checkParticleVelocity( std::integral_constant<int,Order>(),
                           pu_host, pb_host, near_eps, 0 );
    checkParticleVelocity( std::integral_constant<int,Order>(),
                           pu_host, pb_host, near_eps, 1 );
    checkParticleVelocity( std::integral_constant<int,Order>(),
                           pu_host, pb_host, near_eps, 2 );
    ...
    checkGridMomentum( std::integral_constant<int,Order>(),
                       cx, cy, cz, gv_host, near_eps, 0, 0 );
    checkGridMomentum( std::integral_constant<int,Order>(),
                       cx, cy, cz, gv_host, near_eps, 1, 1 );
    checkGridMomentum( std::integral_constant<int,Order>(),
    ...
    auto gm_host = Kokkos::create_mirror_view_and_copy(
        Kokkos::HostSpace(), gm_view );
    checkGridMass( std::integral_constant<int,Order>(),
                   cx, cy, cz, gm_host, near_eps );
}
...
void staggeredTest()
{
    ...
    checkGridVelocity( std::integral_constant<int,Order>(),
                       cx, cy, cz, gs_host, near_eps, Dim, 0 );
