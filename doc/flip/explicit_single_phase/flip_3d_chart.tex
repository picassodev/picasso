...
\item If inviscid, compute artificial viscosity and add to pressure
  \begin{itemize}
  \item
    \[
    \begin{aligned}
    A_c &=
    \begin{cases}
      C^2 dx^2 \rho_c \sum_i (\dic \cdot \ui)^2, & \text{if } \sum_i
      \dic \cdot \ui < 0 \\
      0, & \text{otherwise}
    \end{cases}
    \end{aligned}
    \]
  \item $\pc \leftarrow \pc + A_c$
\end{itemize}
...
\item boundary condition is applied to $\uit$
\begin{itemize}
...
\begin{itemize}
    ...
    \item $ \uio = \left( \uit - (1-\theta)\uiz \right)/\theta $
\end{itemize}

\item {\bf g2p :}update particle velocity, position, deformation, and internal energy
\begin{itemize}
    \item $\epo = \epz \left\{ 1 - \sum_c \pc( \dic \cdot \uit ) \: \pcp \Delta t / \rhoc \icz \right\} \\
    - \mp \left\{ \sum_c \left(\qc (\dic \cdot \uit ) - \Psi_c^0 \cdot ( \dic \times \uit ) \right) \pcp \Delta t / \rhoc^0 \right\} \\
...
\begin{enumerate}[label*=\arabic*.]
    ...
    \begin{itemize}
        ...
        \item $\xpo = \xpz + \sum_i \uit \pip \Delta t$
        \item $j_p = j_p\ exp( \Delta t \sum_i \nabla \cdot \pip \uio )$
    \end{itemize}
\end{enumerate}
